...
class MediaDownloader:
    ...
    def __init__(self, root):
        ...
        self.root.geometry(f"760x620+{px}+{py}")
        self.root.deiconify()
        ...
        FT  = ("Microsoft YaHei UI", 13)
        FT_S = ("Microsoft YaHei UI", 12)
        FT_XS = ("Microsoft YaHei UI", 10)
        FT_H1 = ("Microsoft YaHei UI", 24, "bold")
        FT_H2 = ("Microsoft YaHei UI", 13, "bold")
        ...
        tk.Label(hero, text="媒达", bg=BG, fg=TEXT,
                 font=FT_H1).pack(anchor="w")
        tk.Label(hero, text="媒体下载，快速到达", bg=BG, fg=MUTED,
        ...
        ctk.CTkButton(ctrl, text="打开目录", font=FT_XS, fg_color="transparent",
                      ...
                      height=30, width=72, command=self.open_dir).pack(side="right")
        ctk.CTkButton(ctrl, text="免责声明", font=FT_XS, fg_color="transparent",
                      text_color=MUTED, hover_color="#F5F0E8", corner_radius=8,
                      height=30, width=72, command=self.show_disclaimer).pack(side="right", padx=(0, 4))
        ...
        self.tip_label = tk.Label(main, text="", bg=BG, fg=MUTED,
                                  font=("Microsoft YaHei UI", 10))
        self.tip_label.pack(fill="x", padx=24, pady=(8, 0))
    ...
    def _set_icon(win):
        try:
            if getattr(sys, 'frozen', False):
                ...
                ico = os.path.join(os.path.dirname(os.path.abspath(__file__)), "媒达.ico")
    ...
    def _check_disclaimer(self):
        flag = MediaDownloader._get_flag_path()
        ...
        MediaDownloader._set_icon(dlg)
        dlg.resizable(False, False)
        dlg.configure(bg="#FBF7F2")

        f = tk.Frame(dlg, bg="#FBF7F2", padx=28, pady=28)
        f.pack()

        tk.Label(f, text="免责声明", bg="#FBF7F2", fg="#3D3628",
                 font=("Microsoft YaHei UI", 18, "bold")).pack(anchor="w")

        body = tk.Text(f, bg="#FBF7F2", fg="#5C5042",
                       font=("Microsoft YaHei UI", 12),
                       wrap="word", width=50, height=13,
                       relief="flat", borderwidth=0,
                       padx=0, pady=0,
                       spacing1=4, spacing3=4)
        body.pack(anchor="w", pady=(14, 18))

        body.tag_configure("bold", font=("Microsoft YaHei UI", 12, "bold"))
        body.tag_configure("red", foreground="#C0392B",
                           font=("Microsoft YaHei UI", 12, "bold"))

        body.insert("end", "本工具仅供学习交流使用，", "bold")
        body.insert("end", "严禁用于任何商业或非法用途。", "red")
        body.insert("end", "\n\n")
        body.insert("end", "使用本工具下载的任何音视频内容，其版权归相关权利人所有。请勿将下载内容用于商业用途或未经授权的传播。使用者应确保自身行为符合相关平台的服务条款及当地法律法规。")
        body.insert("end", "\n\n")
        body.insert("end", "本工具不存储任何用户数据，开发者不对使用者的下载行为承担任何直接或间接责任。")
        body.insert("end", "\n\n")
        body.insert("end", "点击「同意」即表示您已阅读并理解以上条款，并愿意", "bold")
        body.insert("end", "自行承担使用本工具的一切责任和后果。", "red")
        ...
        ctk.CTkButton(btns, text="同意", font=("Microsoft YaHei UI", 13),
        ...
        ctk.CTkButton(btns, text="拒绝", font=("Microsoft YaHei UI", 13),
                      fg_color="transparent", text_color="#7A6E5C",
                      hover_color="#F5F0E8", corner_radius=8, height=38,
                      width=80, command=dlg.destroy).pack(side="left")
    ...
    def _start_tips(self):
        self._tip_idx = 0
        self.tip_label.configure(text=self.tips[0])

        def _rotate():
            if not self.is_downloading:
                return
            self._tip_idx = (self._tip_idx + 1) % len(self.tips)
            self.tip_label.configure(text=self.tips[self._tip_idx])
            self._tip_timer = self.root.after(5000, _rotate)
    ...
    def progress_hook(self, d):
        ...
        if d["status"] == "downloading":
            ...
            size_str = self._fmt_size(downloaded)
            if total > 0:
                size_str += f" / {self._fmt_size(total)}"
            info_text = f"下载中... {size_str} · {speed_str}"
            self.root.after(0, lambda p=pct, t=info_text: (
                self.progress_bar.set(p / 100.0),
                self.dl_btn.configure(text=f"下载中 {p:.0f}%"),
                self.info_var.set(t)
            ))
            ...
            self.root.after(0, lambda: (
                ...
                self.dl_btn.configure(text="封装中..."),
    ...
    def _ensure_ffmpeg(self):
        ...
        for base_dir in search_dirs:
            if not base_dir:
                continue
            for candidate in ("ffmpeg.exe", os.path.join("ffmpeg", "ffmpeg.exe")):
                ffmpeg_exe = os.path.join(base_dir, candidate)
            ...
            for chunk in resp.iter_content(chunk_size=8192):
                ...
                if chunk:
                    ...
                    if total > 0:
                        ...
                        t = f"下载中... {self._fmt_size(downloaded)} / {self._fmt_size(total) if total > 0 else '?'}"
                        self.root.after(0, lambda p=pct, s=t: (self.progress_bar.set(p / 100.0), self.info_var.set(s)))
    ...
    def download(self, url, out_dir):
        ...
        # Douyin: cookie first (if enabled) → Playwright fallback → yt-dlp
        tried_playwright = False
        if self._is_douyin_url(url) and not self.audio_only_var.get():
            use_cookies = self.cookie_var.get()
            if use_cookies:
                ydl_opts_temp = self._build_ydl_opts(out_dir)
                if not ydl_opts_temp.get("cookiesfrombrowser") and not ydl_opts_temp.get("cookiefile"):
                    ydl_opts_temp["cookiesfrombrowser"] = ("edge",)
                self.log("--- 方式1: 浏览器 Cookie ---")
                try:
                    with yt_dlp.YoutubeDL(ydl_opts_temp) as ydl:
                        info = ydl.extract_info(url, download=False)
                    self.log("Cookie 有效，使用浏览器身份下载")
                    # Success — drop through to yt-dlp flow below with cookies intact
                except Exception as e:
                    if self._is_cookie_error(str(e)):
                        self.log("  Cookie 无效，尝试 Playwright 解析...")
                        use_cookies = False  # Fall through to Playwright
                    else:
                        self.log(f"  Cookie 方式失败: {e}")

            if not use_cookies or not self.cookie_var.get():
                self.log("--- Playwright 解析 ---" if not use_cookies else "--- Playwright 解析（Cookie 失败后兜底） ---")
                ...
                if result:
                    ...
                    try:
                        ...
                        if "用户取消" in str(e):
                            ...
                            return
                        self.log(f"  [失败] 直链下载失败: {e}")
                else:
                    self.log("  Playwright 未能解析出视频地址")

            if not tried_playwright:
                # Cookies worked, go straight to yt-dlp
                pass
        ...
        while True:
            try:
                ...
                # Use glob to find any existing file with this base name (any extension)
                existing = None
                for fn in os.listdir(out_dir):
                    base, _ = os.path.splitext(fn)
                    if base == safe_title:
                        existing = os.path.join(out_dir, fn)
                        break
                if existing:
                    final_path = self._resolve_file_conflict(existing)
                    if final_path is None:
                        self.root.after(0, lambda: self._on_cancel())
                        return
                    if final_path != existing:
                        new_base = os.path.splitext(os.path.basename(final_path))[0]
                        ydl_opts["outtmpl"] = os.path.join(out_dir, new_base + ".%(ext)s")
...
if __name__ == "__main__":
    if sys.platform == 'win32':
        try:
            import ctypes
            ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID('MeiDa.MediaDownloader.V1.1')
        except Exception:
            pass
